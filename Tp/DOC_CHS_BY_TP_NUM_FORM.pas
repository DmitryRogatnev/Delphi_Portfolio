...
uses
  ...
  ToolCtrlsEh, DBGridEhToolCtrls, DynVarsEh, Vcl.Menus, Data.DB, OracleData,
...
type
  ...
  /// </returns>
  function Get_Doc_Info_By_KeyWord (v_KeyWord :string) :Doc_Info;
  ...
  ///  Doc_Ceh_Kode - Цех / отдел)
  /// </returns>
  function Get_Doc_Info_By_Id (Doc_Id: Integer) :Doc_Info;
  end;
...
{$Region 'Основные функции формы'}
  ...
  function TForm54.Get_Doc_Info_By_KeyWord (v_KeyWord :string) :Doc_Info;
  begin
   //Подстановка ключевого слова
    DB_TP_DOCS.Close;
    DB_TP_DOCS.SetVariable('v_KeyWord', v_KeyWord);
    DB_TP_DOCS.Open;
  ...
  function TForm54.Get_Doc_Info_By_Id (Doc_Id: Integer) :Doc_Info;
  begin
   SimpleSQL.Close;
...
{$Region 'Выбор значения'}
    ...
    procedure TForm54.N1Click(Sender: TObject);
    begin
      Form54.DBGridEh1DblClick(nil);
    end;
    //Нажатие на клавишу Enter
    procedure TForm54.DBGridEh1KeyUp(Sender: TObject; var Key: Word;
    Shift: TShiftState);
    begin
      if key = 13 then
      begin
...
//Двойное нажатие мыши на выбранное строке
    procedure TForm54.DBGridEh1DblClick(Sender: TObject);
    begin
     form54.Close;
    end;
